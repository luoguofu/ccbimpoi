...
public class GestureManager extends GestureDetector.SimpleOnGestureListener
     ...
     * 默认双击放大的时间
     */
    public final static int DOUBLE_SCALE_TIME = 400;
    ...
    public GestureManager(IXImageView xiv, IBitmapManager ibm) {
        mBM = ibm;
        mXImageView = xiv;
    ...
    public boolean onSingleTapConfirmed(MotionEvent e) {
        ...
        if (BitmapManager.DEBUG_BIG)
            L.e("On Tapped: X: " + x + " Y: " + y + " Is: "
                    + (mBM != null && mBM.isTapOnImage(x, y)));
        if (mActionListener != null) {
            mActionListener.onSingleTapped(mXImageView.getInstance(), e,
                    mBM != null && mBM.isTapOnImage(x, y));
        }
        return true;
    }
