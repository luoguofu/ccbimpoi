...
public class BaseImageDecoder implements ImageDecoder {
    ...
    /**
     * Decodes image from URI into {@link Bitmap}. Image is scaled close to incoming
     * {@linkplain ImageSize target size} during decoding (depend on incoming parameters).
     *
    ...
    public Bitmap decode(ImageDecodingInfo decodingInfo) throws IOException {
        ...
        if (imageStream == null) {
            UILL.e(ERROR_NO_IMAGE_STREAM, decodingInfo.getImageKey());
            return null;
        }
        try {
        ...
        if (decodedBitmap == null) {
            UILL.e(ERROR_CANT_DECODE_IMAGE, decodingInfo.getImageKey());
        } else {
            decodedBitmap =
                    considerExactScaleAndOrientatiton(decodedBitmap, decodingInfo,
    ...
    protected ImageFileInfo defineImageSizeAndRotation(InputStream imageStream,
        ...
        if (StrUtil.notEmptyOrNull(imageUri) && new File(imageUri).exists()) {
            if (decodingInfo.shouldConsiderExifParams()
                    && canDefineExifParams(imageUri, options.outMimeType)) {
                exif = ImageUtil.getPictureDegree(imageUri);
            } else {
                exif = new ExifInfo();
            }
        } else {
        ...
        return new ImageFileInfo(new ImageSize(options.outWidth, options.outHeight, exif.rotation),
                exif);
    }
...
//            }
//        } catch (IOException e) {
//            UILL.w("Can't read EXIF tags from file [%s]", imageUri);
//        }
...
//    }
    ...
    protected Options prepareDecodingOptions(ImageSize imageSize, ImageDecodingInfo decodingInfo) {
        ...
        if (scaleType == ImageScaleType.NONE) {
            ...
            scale =
                    ImageSizeUtils.computeImageSampleSize(imageSize, targetSize,
                            decodingInfo.getViewScaleType(), powerOf2);
        }
        if (scale > 1 && loggingEnabled) {
            UILL.d(LOG_SUBSAMPLE_IMAGE, imageSize, imageSize.scaleDown(scale), scale,
    ...
    public Bitmap considerExactScaleAndOrientatiton(Bitmap subsampledBitmap,
        ...
        if (scaleType == ImageScaleType.EXACTLY || scaleType == ImageScaleType.EXACTLY_STRETCHED) {
            ImageSize srcSize =
                    new ImageSize(subsampledBitmap.getWidth(), subsampledBitmap.getHeight(),
                            rotation);
            float scale =
                    ImageSizeUtils.computeImageScale(srcSize, decodingInfo.getTargetSize(),
            ...
            if (Float.compare(scale, 1f) != 0) {
                ...
                if (loggingEnabled) {
                    UILL.d(LOG_SCALE_IMAGE, srcSize, srcSize.scale(scale), scale,
                            decodingInfo.getImageKey());
                }
            }
        }
        ...
        if (flipHorizontal) {
            ...
            if (loggingEnabled) UILL.d(LOG_FLIP_IMAGE, decodingInfo.getImageKey());
        }
        // Rotate bitmap if need
        if (rotation != 0) {
            m.postRotate(rotation);

            if (loggingEnabled) UILL.d(LOG_ROTATE_IMAGE, rotation, decodingInfo.getImageKey());
        }
